...
pub(super) fn show_for_clipboard_item(
    ...
    // 菜单窗口是透明小窗，自己走 show()，此前一直没套过原生材质：
    // 云母/亚克力下前端只上色不模糊，等于直接透出桌面。
    crate::window::apply_window_material(app, CONTEXT_MENU_WINDOW_LABEL);
...
pub fn show_submenu(app: &AppHandle, input: ShowContextSubmenuInput) -> Result<()> {
    ...
    crate::window::apply_window_material(app, CONTEXT_SUBMENU_WINDOW_LABEL);
