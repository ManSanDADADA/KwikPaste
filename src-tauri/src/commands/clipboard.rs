...
pub async fn read_clipboard(
    ...
    // 先在同步段抓前台应用 + 读取剪贴板。
    // 注意：手动「重新读取」时前台应用就是 KwikPaste 自己，不像 OS 监听场景能拿到原应用——
    // 这里仍保留探测：若用户在外部应用复制后立刻命令式触发，多少能捕获到正确来源；
    // 命中我们自己也无害（apps 表只是多记一条「KwikPaste」）。
    let (item_opt, source) = {
...
///
/// 缩略图按需生成：不存在时在阻塞线程池里从原图解码 + 缩放 + 编码后落盘，再返回路径，
/// 并发张数由 `ImageStore` 的信号量限制；「返回时文件已确保存在」，
...
pub async fn get_clipboard_image_path(
    ...
    let path = if thumbnail {
        store.ensure_thumbnail_async(&file_name).await?
    } else {
...
pub(crate) async fn preview_content_metrics(
    ...
    pool: &SqlitePool,
    item_id: &str,
) -> Result<Option<PreviewContentMetrics>> {
...
fn preview_text_rows(item: &ClipboardItem, redact_sensitive: bool) -> u32 {
...
/// 统计软切后的虚拟行数，规则与前端 `buildTextPreviewRows` 一致；`masked` 为真时按
/// 遮罩后的文本统计。
///
...
fn count_preview_text_rows(source: &str, masked: bool) -> u32 {
    let masked_text;
    let text = if masked {
...
/// 为 image 条目补齐**已存在**的缩略图绝对路径，前端可直接渲染。
/// 缩略图尚未生成时返回 `None`：前端先显示同尺寸占位，再按需调 `get_clipboard_image_path`
/// 生成；不再回退到原图路径，避免 WebView 为一个几十像素高的卡片解码整张原图。
