...
pub use window::MaterialSupport;
...
/// 返回当前系统对各原生窗口材质的支持情况；前端据此禁用不可用选项并回退纯色。
#[tauri::command]
pub async fn get_window_material_support() -> MaterialSupport {
    window::material_support()
}
...
pub async fn notify_window_ready(app: AppHandle, label: String) {
    ...
    // 预览窗口冷启动时把显示挂起到这里：页面挂好了再放出来，不让用户看到空面板。
    if label == window::CLIPBOARD_PREVIEW_WINDOW_LABEL {
        window::preview::on_preview_window_ready(&app);
    }
}
...
/// 预览窗口就是面板本身，尺寸必须在显示前定好，所以这里先按记录算出内容度量再交给窗口层。
/// 记录已被删除时度量为 `None`，窗口层按空状态的最小面板显示。
#[tauri::command]
pub async fn show_clipboard_preview(
    ...
    let metrics =
        crate::commands::clipboard::preview_content_metrics(&app, &pool, &item_id).await?;

    window::preview::show_clipboard_preview(&app, item_id, anchor, metrics)
}
