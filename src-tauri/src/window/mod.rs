...
pub use material::MaterialSupport;
...
pub fn apply_clipboard_window_material(
    ...
    material::apply(app_handle, CLIPBOARD_WINDOW_LABEL, appearance)
}

/// Synchronizes the persisted material to every currently-created native window.
pub fn apply_existing_window_material(app_handle: &AppHandle, appearance: &Appearance) {
    material::apply_existing(app_handle, appearance);
}

/// 按当前外观设置给指定窗口套原生材质。供不走 [`show_window`] 的自定义显示路径
/// （如右键菜单窗口自己定位后 `show()`）在显示前调用；失败只记日志，不挡显示。
pub fn apply_window_material(app_handle: &AppHandle, label: &str) {
...
pub fn round_popup_corners(window: &WebviewWindow) {
    #[cfg(target_os = "windows")]
...
/// Reports which native materials the current OS can render.
pub fn material_support() -> MaterialSupport {
    material::support()
}
