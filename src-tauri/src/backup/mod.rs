//! KwikPaste 历史备份包导出与接收壳识别。
//!
//! `.kwikpastebak` 有两种格式：明文模式是标准 ZIP；加密模式是 KwikPaste 自有容器。
...
/// 导出当前环境历史数据库、资源文件和设置为 `.kwikpastebak` 备份包。
...
/// 从 `.kwikpastebak` 导入历史和设置；合并写入当前库，覆盖热替换当前数据。
...
/// 从进程参数中查找 `.kwikpastebak` 路径，供 Windows 文件关联和第二实例回调使用。
