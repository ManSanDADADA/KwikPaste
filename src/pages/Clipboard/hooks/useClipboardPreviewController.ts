...
interface PreviewPointerTarget {
  item: ClipboardItem;
  pointerX: number;
}
...
export function useClipboardPreviewController(
  ...
  const handleItemPointerEnter = (
    ...
    hoverTimerRef.current = window.setTimeout(() => {
      ...
      void openPreviewForItem(target.item, "hover", target.pointerX);
    }, HOVER_DELAY_MS[previewSettings.hoverDelayMs]);
  };

  /**
   * 延迟打开期间跟踪指针位置；预览已经打开后同一卡片内的移动不再重定向窗口——
   * 面板锚定卡片，跟着指针挪意味着每帧移动一个真实窗口。
   */
  const handleItemPointerMove = (
    ...
    pendingHoverTargetRef.current = { item, pointerX: event.clientX };
  };
