...
export interface PreviewAnchorRect {
  left: number;
  /** 指针在剪贴板 webview 里的水平位置，决定面板往左还是往右弹；键盘预览不带。 */
  pointerX?: number;
  top: number;
  width: number;
  height: number;
...
/**
 * 预览窗口的状态。窗口几何全部由 Rust 算好并直接作用在窗口上，
 * 前端只需要知道渲染哪条记录，以及面板落在卡片的哪一侧（决定展开方向）。
 */
...
/**
 * 导出历史数据库、资源和设置为 `.kwikpastebak` 备份包。
 */
...
/**
 * 识别 `.kwikpastebak` 文件并广播给偏好页导入接收壳。
 */
...
/**
 * 按需生成并返回图片缩略图路径。列表首次展示某张图时由卡片调用；
 * 失败只记日志并返回 null，由卡片保持占位，不弹错误提示打断浏览。
 */
export const getClipboardImageThumbnailPath = async (
  fileName: string,
): Promise<string | null> => {
  try {
    return await invoke<string>(TAURI_COMMAND.GET_CLIPBOARD_IMAGE_PATH, {
      fileName,
      thumbnail: true,
    });
  } catch (error) {
    log.error("get clipboard image thumbnail path failed", toAppError(error));
...
/**
 * 读取当前系统对原生窗口材质的支持情况。系统版本在进程内不变，启动时读一次即可；
 * 失败按全部不支持处理，让窗口回退纯色而不是透出未模糊的桌面。
 */
export const getWindowMaterialSupport =
  async (): Promise<WindowMaterialSupport> => {
    try {
      return await invoke<WindowMaterialSupport>(
        TAURI_COMMAND.GET_WINDOW_MATERIAL_SUPPORT,
      );
    } catch (error) {
      log.error("get window material support failed", toAppError(error));

      return { acrylic: false, mica: false };
    }
  };
